...
|----------|-------------|
| `GEOSEAROUTE_BASE_URL` | Default base URL for the target geosearoute service |
| `GEOSEAROUTE_API_KEY` | Default API key for authenticated service calls |

### Override Rules

| Input | Behavior |
|-------|----------|
| `--base-url` | Overrides `GEOSEAROUTE_BASE_URL` for the current invocation |
| `--api-key` | Overrides `GEOSEAROUTE_API_KEY` for the current invocation |
| Missing both env and CLI value | Command fails before any HTTP request |
...
| `--base-url` | string | env/default | Override service base URL |
| `--api-key` | string | env/default | Override API key |
| `--help` | flag | - | Show command help |

### Remote Request Contract

```http
GET /nearest?lat=<lat>&lon=<lon>&distance=<distance>
```
...
```bash
$ geosearoute-cli nearest 57.7 11.9
{
  "point": {
    "lat": 57.701,
    "lon": 11.902
...
```bash
$ geosearoute-cli solve --stop 11.9 57.7 --stop 4.48 51.92 --speed 20
{
  "distance_nm": 423.1,
  "duration_hours": 21.2,
  "geometry": {
...
```text
Error: Missing geosearoute API key.
Provide GEOSEAROUTE_API_KEY or pass --api-key.
```
...
configuration:
  GEOSEAROUTE_BASE_URL   Default service base URL
  GEOSEAROUTE_API_KEY    Default API key
```
...
**Configuration precedence**: CLI flags override environment defaults; missing required config fails early.  
